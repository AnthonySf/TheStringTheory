...
public static class AlphaTabSourceResolver
{
    public static AlphaTabResolvedSourceData Resolve(string notationPath, int trackIndex)
    {
        ...
        if (RocksmithCachedSongLoader.IsRocksmithManifestPath(normalizedPath))
        {
...
public static class RocksmithAlphaTabSourceBuilder
{
    private const string CacheFolderName = "RocksmithSources";
    private const string ExportVersion = "v13";
    private const int MinimumAlphaTabSchemaVersion = 14;

    public static string GetOrCreate(string manifestPath, int arrangementIndex)
    {
        if (string.IsNullOrWhiteSpace(manifestPath))
            throw new ArgumentException("Manifest path was empty.", nameof(manifestPath));
        ...
        RocksmithMusicXmlWriter.Write(outputPath, manifest, summary, part);
        return outputPath;
    }
        ...
        out RocksmithCachedArrangementPart part)
    {
        ...
        if (HasTimingData(part?.timing) && (part?.schemaVersion ?? 0) >= MinimumAlphaTabSchemaVersion)
            return true;
        ...
        if (!HasTimingData(part?.timing))
            throw new InvalidOperationException("Rocksmith refresh completed, but the imported arrangement still does not contain AlphaTab timing data.");
        if ((part?.schemaVersion ?? 0) < MinimumAlphaTabSchemaVersion)
            throw new InvalidOperationException($"Rocksmith refresh completed, but the imported arrangement schema ({part?.schemaVersion ?? 0}) is older than the AlphaTab minimum ({MinimumAlphaTabSchemaVersion}).");

        return true;
    }

    private static bool HasTimingData(RocksmithCachedArrangementTimingData timing)
    {
        return timing != null &&
               timing.ebeats != null &&
               timing.ebeats.Count > 0;
    }

    private static long GetSourceTicks(string manifestPath, string partFilePath)
    {
        long manifestTicks = File.Exists(manifestPath) ? File.GetLastWriteTimeUtc(manifestPath).Ticks : 0L;
        long partTicks = !string.IsNullOrWhiteSpace(partFilePath) && File.Exists(partFilePath)
            ? File.GetLastWriteTimeUtc(partFilePath).Ticks
            : 0L;
        return Math.Max(manifestTicks, partTicks);
    }

    private static string ComputeCacheKey(string manifestPath, string partId, long sourcePsarcTicks)
    {
        string input = string.Join("|", new[]
        {
            ExportVersion,
            Path.GetFullPath(manifestPath).ToLowerInvariant(),
            partId ?? string.Empty,
            sourcePsarcTicks.ToString(CultureInfo.InvariantCulture)
        });
...
public static class RocksmithAlphaTabGpSourceBuilder
{
    ...
    public static string GetOrCreate(string manifestPath, int arrangementIndex)
    {
        ...
        RocksmithAlphaTabGpWriter.Write(outputGpPath, manifest, summary, part);
        return outputGpPath;
    }
    ...
    {
        long manifestTicks = File.Exists(manifestPath) ? File.GetLastWriteTimeUtc(manifestPath).Ticks : 0L;
        string resolvedPartPath = ResolveRelativeToManifest(manifestPath, partFilePath);
        long partTicks = !string.IsNullOrWhiteSpace(resolvedPartPath) && File.Exists(resolvedPartPath)
            ? File.GetLastWriteTimeUtc(resolvedPartPath).Ticks
            : 0L;
        return Math.Max(manifestTicks, partTicks);
    }

    private static string ResolveRelativeToManifest(string manifestPath, string candidatePath)
    {
        ...
        return Path.Combine(manifestDirectory, candidatePath);
    }
