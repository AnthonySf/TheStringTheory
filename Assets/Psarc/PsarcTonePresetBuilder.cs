...
public static class PsarcTonePresetBuilder
{
    public const string GeneratedPresetIdPrefix = "rsgen_";

    private const float DefaultKnobValue = 0.5f;
    private const float PsarcReferenceToneVolumeDb = -20f;
    private const float MinPsarcToneVolumeDb = -45f;
    private const float MaxPsarcToneVolumeDb = 45f;
        ...
        out UnityToneLabRuntime.ToneLabPreset preset)
    {
        ...
        if (string.IsNullOrWhiteSpace(rawToneJson) ||
            !PsarcToneJsonParser.TryParse(rawToneJson, out object parsed) ||
        ...
        List<PsarcGear> gears = ExtractGears(gearList);
        if (gears.Count == 0)
            return false;
    ...
    private static List<PsarcGear> ExtractGears(Dictionary<string, object> gearList)
    {
        List<PsarcGear> gears = new List<PsarcGear>();
        if (gearList == null)
            return gears;
    ...
    private static void AddGearIfUsable(List<PsarcGear> gears, string slotKey, Dictionary<string, object> gearObject)
    {
        ...
        float? rootToneVolumeDb)
    {
        ...
        if (gears != null)
        {
            foreach (PsarcGear gear in gears)
            {
                if (gear == null)
                    continue;
        ...
        outputGain = ApplyPsarcToneVolume(outputGain, rootToneVolumeDb);
    ...
    private static float? TryGetRootToneVolumeDb(Dictionary<string, object> root)
    {
        ...
        return Mathf.Clamp(volumeDb, MinPsarcToneVolumeDb, MaxPsarcToneVolumeDb);
    }

    private static float ApplyPsarcToneVolume(float outputGainDb, float? psarcToneVolumeDb)
    {
        if (!psarcToneVolumeDb.HasValue)
            return outputGainDb;

        // Psarc tone volume is inverted in Toolkit/exported tones: 0 is soft, -20 is normal, -30 is loud.
        float correctionDb = PsarcReferenceToneVolumeDb - psarcToneVolumeDb.Value;
        return Mathf.Clamp(outputGainDb + correctionDb, MinGeneratedOutputGainDb, MaxGeneratedOutputGainDb);
    }

    private static string BuildGearSearchText(IReadOnlyList<PsarcGear> gears)
    {
    ...
    private static List<UnityToneLabRuntime.ToneLabPedalSlot> BuildPedalChain(
        ...
        Func<string, bool> lv2PluginAvailable)
    {
        bool useLv2GeneratedChain = PsarcToneLv2Mappings.AreRequiredPluginsAvailable(lv2PluginAvailable);
        ...
        foreach (PsarcGear gear in gears)
        {
            GearFamily family = ClassifyGear(gear);
            CanonicalControls controls = CanonicalControls.FromKnobs(gear.Knobs);
            if (useLv2GeneratedChain && PsarcToneLv2Mappings.TryCreateMappedSlot(
                    gear.SlotKey,
                    gear.Type,
                    gear.Name,
                    gear.Category,
                    ...
                    context.HighGain,
                    context.DriveIntent,
                    out PsarcToneLv2SlotMapping mappedSlot))
            {
                switch (mappedSlot.Role)
                {
                    case PsarcToneLv2SlotRole.Dynamics:
                        ...
                        break;
                    case PsarcToneLv2SlotRole.Gain:
                        ...
                        break;
                    case PsarcToneLv2SlotRole.Amp:
                        ampSlot ??= mappedSlot.Slot;
                        break;
                    case PsarcToneLv2SlotRole.Cab:
                        cabSlot ??= mappedSlot.Slot;
                        break;
                    case PsarcToneLv2SlotRole.Eq:
                        eq.Add(mappedSlot.Slot);
                        break;
                    case PsarcToneLv2SlotRole.Modulation:
                        modulation.Add(mappedSlot.Slot);
                        break;
                    case PsarcToneLv2SlotRole.Ambience:
        ...
        if (context.NeedsGate && !hasGate)
        {
            chain.Add(useLv2GeneratedChain
                ? PsarcToneLv2Mappings.CreateDefaultNoiseGateSlot(context.DriveIntent, context.HighGain)
        ...
        chain.Add(ampSlot ?? (useLv2GeneratedChain
            ? PsarcToneLv2Mappings.CreateDefaultAmpSlot(
                context.IsBass,
                context.HighGain,
                context.DriveIntent,
                context.BassIntent,
        ...
        UnityToneLabRuntime.ToneLabPedalSlot defaultCab = cabSlot ?? (useLv2GeneratedChain
            ? PsarcToneLv2Mappings.CreateDefaultCabSlot(
                context.IsBass,
                context.HighGain,
                context.DriveIntent,
                context.BassIntent,
        ...
        {
            chain.Add(useLv2GeneratedChain
                ? PsarcToneLv2Mappings.CreateDefaultEqSlot(
                    context.IsBass,
                    context.HighGain,
                    context.BassIntent,
                    context.MidIntent,
        ...
        {
            chain.Add(useLv2GeneratedChain
                ? PsarcToneLv2Mappings.CreateDefaultReverbSlot(context.HighGain)
    ...
    private static GearFamily ClassifyGear(PsarcGear gear)
    {
    ...
    private static bool LooksLikeAmpGear(PsarcGear gear)
    {
    ...
    {
        return string.IsNullOrWhiteSpace(toneName) ? "PSARC Tone" : toneName.Trim();
    }
    ...
    private sealed class PsarcGear
    {
    ...
    private sealed class PsarcToneJsonParser
    {
        private readonly string json;
        private int index;

        private PsarcToneJsonParser(string json)
        {
        ...
        public static bool TryParse(string json, out object value)
        {
            ...
            try
            {
                PsarcToneJsonParser parser = new PsarcToneJsonParser(json);
