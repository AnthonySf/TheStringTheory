...
float4 SampleAurora(float3 uv) 
{
    ...
    float painterShift = saturate(0.5 + 0.5 * sin(uv.x * 5.8 + uv.y * 2.1 + _Time.y * _AuroraSpeed * 9.0));
    float3 sampledAuroraColor = _AuroraColor.rgb * cloudColor.rgb;
    sampledAuroraColor = lerp(sampledAuroraColor, float3(0.58, 0.22, 1.00), (1.0 - painterShift) * 0.18);
    sampledAuroraColor = lerp(sampledAuroraColor, float3(1.00, 0.18, 0.62), painterShift * 0.26);
    aurora.rgb *= lerp(float3(0,0,0), sampledAuroraColor * _AuroraBrightness, contrastMask);
...
{     
	...
	float auroraPlacementMask = smoothstep(0.015, 0.09, viewDir.y) * (1.0 - smoothstep(0.46, 0.78, viewDir.y));
	float3 traceDir = normalize(viewDir + float3(0, viewFalloff * 0.12 ,0));
	...
	finalColor *= saturate(viewDir.y * 4.5) * auroraPlacementMask;
